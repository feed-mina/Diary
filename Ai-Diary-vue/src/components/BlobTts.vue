...
const readJapanese = async () => {
  ...
  try {
    ...
    const readNextLine = async () => {
      ...
      let lineText = titleAndContentLines.value[lineIndex].trim()
      console.log("현재 읽을 줄:", lineText)

      if (!lineText || lineText.length < 2) {
        console.warn("너무 짧아서 건너뜀:", lineText)
        lineIndex++
        currentLineIndex.value = lineIndex
        setTimeout(() => readNextLine(), 200)
        return
      }

      const response = await api.post('/tts_blob', { text: lineText }, { responseType: 'blob' })
      if (!response || response.status !== 200) {
        console.error('TTS 응답 실패:', response)
        isReading.value = false
        return
      }
      ...
      audio.onerror = (e) => {
        ...
        isReading.value = false
        alert("오디오 생성에 실패했습니다. 해당 줄을 건너뜁니다.")
      }

      const baseTimePerCharacter = 150
