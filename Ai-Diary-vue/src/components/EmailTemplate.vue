...
export default {
  ...
  methods: {
    async sendEmail() {
      try {
        const response = await  axiosInstance.post(`/api/auth/send`, this.email);
        alert(response.data);
      } catch (error) {
        console.error(error);
        alert("이메일 전송 실패");
    ...
    async semdHtmlEmail() {
      try {
        const response = await  axiosInstance.post(`/api/email/send-html`, {
    ...
    async sendEmailWithAttachment() {
      ...
      try {
        const response = await  axiosInstance.post(`/api/email/send-attachment`, formData);
        alert(response.data);
      } catch (error) {
        console.error(error);
        alert('Failed to send email.');
